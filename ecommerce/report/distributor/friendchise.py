...
from ecommerce.report.sale import AgFrBoughtSummaryReport
from member.models import Member


class FriendchiseBoughtReport(ExcelTemplateReport, MonthMixIn):
    ...
    def __init__(self, *args, **kwargs):
        super(FriendchiseBoughtReport, self).__init__(**kwargs)
        ...
        self.bough_history = AgFrBoughtSummaryReport(start=self.start, end=self.end, get_type='monthly',
                                                     type=('FR', 'MF'))
        self.transfer_history = FrPvTransferInReport(start=self.start, end=self.end, get_type='monthly')
    ...
    def build_row_meta(self, row_index, data, **kwargs):
        ...
        for x in month_range:
            ...
            pvt_month = data['transfer'].get(current_month, 0)
            total_pv = pvt_month + pv_month
            if total_pv >= self.margin:
    ...
    def process_data(self):
        ...
        t_total = self.transfer_history.total
        merge_pool = {}
        for x in Member.objects.filter(group__code__in=('MF', 'FR')):
            merge_pool[x.code] = {
                "code": x.code,
                "name": x.full_name,
                "data": {},
