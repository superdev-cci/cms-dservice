...
from core.report.pdf.flowable.transfer_item import TransferItem
from core.report.pdf.page_template import InvoiceDocTemplate
from core.utility import num_to_text
from branch.models.branch_transfer_hq import BranchTransferHqItem


class InvoiceTransferHqPdf(InvoiceDocTemplate):
    page_size = A4
    margin_buttom = (1 * cm)
    ...
    table_head = [
        ...
        {'name': 'รายการ', 'cursor': (12.4 * cm, 0)},
        {'name': 'จำนวน', 'cursor': (1.9 * cm, 0)},
    ]

    def __init__(self, file_name, data=None, *args, **kwargs):
        self.branch_obj = None
        super(InvoiceTransferHqPdf, self).__init__(file_name, data=data, **kwargs)
        return

    def get_invoice_type(self):
        return {
            'font': ('Helvetica', 20),
            'name': (0.5 * cm, 3.15 * cm, 'ใบส่งของสาขา')
        }

    def get_head_info(self):
        # self.customer = self.invoice
    ...
    def latest_page(self, *args, **kwargs):
        super(InvoiceTransferHqPdf, self).latest_page(*args, **kwargs)
        self.canv.saveState()
    ...
    def render_head(self):
        ...
        self.canv.drawString(self.width + 15, -(1.3 * cm), 'ใบส่งของสาขา')
        ...
        self.canv.drawImage(self.logo_path, 55, -75, width=75, height=70)
        self.canv.restoreState()
        return
    ...
    def render_customer(self):
        ...
        start_y = -4 * cm
        start_x_header = 1 * cm
        # self.canv.drawString(start_x_header, start_y, 'รหัสสาขา')
    ...
    def render_bill_header(self):
        ...
        start_y -= 0.7 * cm
        self.canv.drawString(start_x_header, start_y, 'ประเภท')
    ...
    def render_barcode(self):
        ...
        self.canv.setFont('ANGSA', 14)
        self.canv.drawString(10 * cm, 5 * cm, 'ผู้ทำรายการ {}'.format(self.invoice.uid))
    ...
    def create_pdf(self, response=None):
        item_list = []
        goods = BranchTransferHqItem.objects.filter(sano=self.invoice.id)
        ...
        for x in goods:
            total_item_count = 1
            total_item = [TransferItem(no=row_count,
                                       code=x.pcode,
                                       item=x.pdesc,
                                       qty=x.qty)]
